    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            function setActiveMenuItem(clickedItem) {
                ...
                localStorage.setItem('activeMenu', clickedItem.querySelector('a').getAttribute('href'));
            }

            // Khởi tạo active dựa trên URL hiện tại hoặc localStorage
            const currentPath = window.location.pathname;
            const savedActiveMenu = localStorage.getItem('activeMenu');
            menuItems.forEach(item => {
                const link = item.querySelector('a').getAttribute('href');
                if (link === currentPath || (savedActiveMenu === link && !currentPath.includes('/edit'))) {
            ...
            menuItems.forEach(item => {
                item.addEventListener('click', function (e) {
                    ...
                    // Ngăn chặn hành vi mặc định nếu href là "#"
                    if (href === '#') {
                        e.preventDefault();
                    }
                    ...
                    // Nếu href không phải "#", cho phép điều hướng
                    if (href !== '#') {
                        setTimeout(() => {
                            window.location.href = href;
                        }, 100); // Thêm độ trễ nhỏ để animation mượt hơn
                    }
                });
            });
